...
def get_latency_analytics(
    ...
    ).filter(Request.timestamp >= start, Request.total_latency_ms.isnot(None))
    if model:
    ...
    data = [
        LatencyBucket(bucket=r.bucket, avg_latency_ms=round(r.avg_latency or 0, 2),
                       min_latency_ms=r.min_latency or 0, max_latency_ms=r.max_latency or 0,
    ...
    stage_avg = (
        db.query(Trace.stage_name, func.avg(Trace.duration_ms).label("avg_duration"))
    ...
    summary = LatencySummary(
        avg_latency_ms=avg_latency, total_requests=total_requests,
...
def get_cost_analytics(
    ...
    start = _period_start(period)
    base_query = db.query(Request).filter(Request.timestamp >= start, Request.cost_usd.isnot(None))
    total_cost = base_query.with_entities(func.sum(Request.cost_usd)).scalar() or 0.0

    by_model_rows = (
        db.query(Request.model_name, func.count(Request.id).label("requests"),
                  func.sum(Request.cost_usd).label("cost"))
        .filter(Request.timestamp >= start, Request.cost_usd.isnot(None))
        .group_by(Request.model_name).all()
    )
    ...
    bucket_expr = _hour_bucket_expr()
    hourly_rows = (
        db.query(bucket_expr.label("bucket"), func.sum(Request.cost_usd).label("cost"),
                  func.count(Request.id).label("requests"))
        .filter(Request.timestamp >= start, Request.cost_usd.isnot(None))
        .group_by(bucket_expr).order_by(bucket_expr).all()
    )
    by_hour = [
        CostBucket(bucket=r.bucket, cost_usd=round(r.cost or 0, 6), requests=r.requests,
                   avg_cost_per_request=round((r.cost or 0) / r.requests, 6) if r.requests else 0.0)
        for r in hourly_rows
    ]

    return CostAnalyticsResponse(period=period, total_cost_usd=round(total_cost, 6),
                                   by_model=by_model, by_hour=by_hour)
...
def get_performance_analytics(
    ...
    success_count = db.query(func.count(Request.id)).filter(
        Request.timestamp >= start, Request.status == "success").scalar() or 0
    error_count = db.query(func.count(Request.id)).filter(
        Request.timestamp >= start, Request.status == "error").scalar() or 0
    ...
    hourly_counts = (
        ...
        .filter(Request.timestamp >= start).group_by(bucket_expr).all()
    )
    counts = [r.cnt for r in hourly_counts] or [0]
    throughput = ThroughputInfo(
        requests_per_hour=round(total_requests / hours_in_period, 2),
        peak_hour_count=max(counts), low_hour_count=min(counts),
    )

    # Errors by which STAGE failed (parsed from error_message prefix)
    error_rows = (
    ...
    # Errors by CATEGORY (from the rule-based failure classifier)
    category_rows = (
        db.query(Request.failure_category, func.count(Request.id))
        .filter(Request.timestamp >= start, Request.status == "error",
                Request.failure_category.isnot(None))
        .group_by(Request.failure_category).all()
    )
    by_category: Dict[str, int] = {cat: count for cat, count in category_rows}

    errors = ErrorBreakdown(total_errors=error_count, by_stage=by_stage, by_category=by_category)

    eval_agg = (
        db.query(func.avg(Evaluation.hallucination_score), func.avg(Evaluation.faithfulness_score),
                  func.avg(Evaluation.answer_relevancy))
        .join(Request, Evaluation.request_id == Request.id)
        .filter(Request.timestamp >= start).first()
    )
    ...
    quality_counts = (
        ...
        .join(Request, Evaluation.request_id == Request.id)
        .filter(Request.timestamp >= start).group_by(Evaluation.quality_level).all()
    )
    ...
    needs_review_count = (
        ...
        .join(Request, Evaluation.request_id == Request.id)
        .filter(Request.timestamp >= start, Evaluation.needs_review == True).scalar() or 0
    )
    ...
    quality = QualityMetrics(
        ...
        excellent_count=level_counts.get("excellent", 0), good_count=level_counts.get("good", 0),
        fair_count=level_counts.get("fair", 0), poor_count=level_counts.get("poor", 0),
        needs_review_count=needs_review_count,
    )

    return PerformanceAnalyticsResponse(
        period=period, total_requests=total_requests, success_rate=success_rate,
        error_rate=error_rate, throughput=throughput, errors=errors, quality=quality,
    )
